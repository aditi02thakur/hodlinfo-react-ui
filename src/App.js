import hodlinfo from '../src/images/hodlinfo.png';
import cryptonews from '../src/images/cryptonews.png';
import banner from '../src/images/Banner.png';
import './App.css';
import Table from './table';

function App() {
  return (
    <div className="App">
    <div className="top">
    <img src={hodlinfo} className="applogo" alt="logo" />
    <div className="btngrp">
    <button className="btnn1" ><a >INR</a></button>
    <button className="btnn2"><a > BTC</a></button>
    <button className="btnn3"><a>BUY BTC</a></button>
    </div>
    <button className="telegram" >
    <div className="boxtele">
      <img src="data:image/png;base64," className="teleimg"/>
    </div>
    <h2 className="contel">Connect Telegram</h2></button>
    </div>
    <div className="bptt"><h1 className="bptt2">Best Price to Trade</h1></div>
    
    <div className="aaa">
      <div className="a111">
        <div className="headd">0.76%</div>
        <div className="subheadd">5mins</div>
      </div>
      <div className="a222">
        <div className="headd">0.76%</div>
        <div className="subheadd">5mins</div>
      </div>
      <div className="a333">
        <div className="headdd"><h1>Rs 4336</h1></div>
        <div className="subheadd">Average BTC/INR net price including commission</div>
      </div>
      <div className="a444">
        <div className="headd">0.76%</div>
        <div className="subheadd">5mins</div>
      </div>
      <div className="a555">
        <div className="headd">0.76%</div>
        <div className="subheadd">5mins</div>
      </div>
    </div>
    <div className="cryptoimg">
    <img src={cryptonews} className="cryptoimgg" alt="img" />
    </div>
    <div className="tablee">
      <Table></Table>
    </div>
    <div className="banner">
    <img src={banner} className="bannerimg" alt="img" />
    </div>


    </div>
  );
}

export default App;
